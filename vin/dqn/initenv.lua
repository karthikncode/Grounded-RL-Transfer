...
function setup(_opt)
    ...
    -- Get the list of games to be played. 
    local game_ids = {}
    for _, id in  pairs(split(opt.game_num, "%d+")) do 
        table.insert(game_ids, tonumber(id))
    end 

      -- Get the list of test levels to be played. 
    local test_game_ids = {}
    for _, id in  pairs(split(opt.test_game_num, "%d+")) do 
        table.insert(test_game_ids, tonumber(id))
    end 

    -- Get the list of levels to be played. 
    local level_ids = {}
    for _, id in  pairs(split(opt.level_num, "%d+")) do 
        table.insert(level_ids, tonumber(id))
    end 

     -- Get the list of test levels to be played. 
    local test_level_ids = {}
    for _, id in  pairs(split(opt.test_level_num, "%d+")) do 
        table.insert(test_level_ids, tonumber(id))
    end 
    ...
    local screen, reward, terminal = gameEnv:newGame(game_ids[1], level_ids[1])
    local objects_per_cell = screen:size(1)
    local x_dim = screen:size(2)
    local y_dim = screen:size(3)
    while not terminal do
        screen, reward, terminal = gameEnv:step(0) -- dummy action
    end

    -- agent options
    _opt.agent_params.game_ids  = game_ids
    _opt.agent_params.level_ids = level_ids
    _opt.agent_params.test_game_ids  = test_game_ids
    _opt.agent_params.test_level_ids = test_level_ids
    ...
    _opt.agent_params.objects_per_cell = objects_per_cell
    _opt.agent_params.pretrained_embeddings = _opt.pretrained_embeddings
    ...
    return game_ids, test_game_ids, level_ids, test_level_ids, gameEnv, gameActions, agent, opt
end

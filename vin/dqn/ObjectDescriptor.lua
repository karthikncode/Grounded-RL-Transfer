...
function ObjectDescriptor:updateOutput(input)
    ...
    for i=1, math.min(self.limit, obj_ids:size(1)) do 
        ...
        -- print("MASK:", obj_ids[i], state,mask)
        -- io.read()
        -- emb_state:cmul(1 - mask)
        emb_state = emb_state + obj_emb:cmul(mask)    -- add up representations for different descriptions of the same object    
        -- end
    end
...
function ObjectDescriptor:updateGradInput(input, gradOutput)
    ...
    -- Don't impose limits for multiple descriptions case as well as for batching case.

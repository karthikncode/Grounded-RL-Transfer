...
function trans:__init(args)
    ...
    self.id = torch.LongTensor(self.maxSize):fill(0)
    self.level_id = torch.LongTensor(self.maxSize):fill(0)
    self.expert_a = torch.LongTensor(self.maxSize):fill(0)
    ...
    self.buf_expert_a      = torch.LongTensor(self.bufferSize):fill(0)
    self.buf_id      = torch.LongTensor(self.bufferSize):fill(0)
    self.buf_level_id      = torch.LongTensor(self.bufferSize):fill(0)
...
function trans:fill_buffer()
    ...
    for buf_ind=1,self.bufferSize do
        local s, a, r, s2, term, id, level_id, expert_a = self:sample_one(1)
        self.buf_s[buf_ind]:copy(s)
        self.buf_a[buf_ind] = a
        self.buf_expert_a[buf_ind] = expert_a
        self.buf_r[buf_ind] = r
        self.buf_s2[buf_ind]:copy(s2)
        self.buf_term[buf_ind] = term
        self.buf_id[buf_ind] = id
        self.buf_level_id[buf_ind] = level_id
    end
...
function trans:sample(batch_size)
    ...
    local buf_s, buf_s2, buf_a, buf_r, buf_term, buf_id, buf_level_id, buf_expert_a = self.buf_s, self.buf_s2,
        self.buf_a, self.buf_r, self.buf_term, self.buf_id, self.buf_level_id, self.buf_expert_a
    if self.gpu and self.gpu >=0  then
        buf_s = self.gpu_s
        buf_s2 = self.gpu_s2
    end

    return buf_s[range], buf_a[range], buf_r[range], buf_s2[range], buf_term[range], buf_id[range], buf_level_id[range], buf_expert_a[range]
end
...
function trans:get(index)
    ...
    return s, self.a[ar_index], self.r[ar_index], s2, self.t[ar_index+1], self.id[ar_index], self.level_id[ar_index], self.expert_a[ar_index]
end


function trans:add(s, a, r, term, id, level_id, expert_a)
    assert(s, 'State cannot be nil')
    assert(a, 'Action cannot be nil')
    assert(r, 'Reward cannot be nil')
    assert(id, 'ID cannot be nil')
    assert(level_id, 'Level ID cannot be nil')
    ...
    self.id[self.insertIndex] = id
    self.level_id[self.insertIndex] = level_id
    self.expert_a[self.insertIndex] = expert_a
    if term then

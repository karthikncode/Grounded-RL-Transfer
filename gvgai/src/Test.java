...
import controllers.singlePlayer.zmq.Agent;
...
public class Test
{
    ...
    public static void main(String[] args)
    {
        ...
        Option port = new Option("p", "port", true, "port for connecting to agent");
        port.setRequired(true);
        options.addOption(port);

        Option gameIndex = new Option("g", "gameIndex", true, "index of game to play");
        gameIndex.setRequired(false);
        options.addOption(gameIndex);
        ...
        // Set the appropriate variable to the right port number.
        Communication.port = Integer.parseInt(cmd.getOptionValue("port"));
        ...
        String zmqController = "controllers.singlePlayer.zmq.Agent";
        ...
        games = new String[]{"aliens", "angelsdemons", "assemblyline", "avoidgeorge", "bait", //0-4
                ...
                "zelda", "zenpuzzle", "game1", "game2", "game3",                              //90-94
                "game4", "game5", "plaque" };                                                 //95-99
        ...
        int gameIdx = 38;// Integer.parseInt(cmd.getOptionValue("gameIndex"));
        ...
        if (human) {
            ArcadeMachine.playOneGame(game, level1, recordActionsFile, seed.nextInt());
        }
        else {
            // Setup ZMQ, etc.
            if (Agent.socket == null) {
                ...
                Agent.socket = ctx.createSocket(ZMQ.REP);
                Agent.socket.bind("tcp://*:" + Communication.port);
                System.out.println("Waiting for client to connect on port " + Communication.port + "...");
            }

            // Play the games.
            while (true) {

                msg = ZMsg.recvMsg(Agent.socket);
                content = msg.pop();
                String msgContent = content.toString();
                assert (msgContent.contains("newGame"));

                // Get the game id.
                int gameID = Integer.parseInt(msgContent.split(" ")[1]);
                int level;
                try {
                    level = Integer.parseInt(msgContent.split(" ")[2]);
                } catch (Exception e){
                    level = 0;
                }
                String gameString = gamesPath + games[gameID] + ".txt";
                String levelString = gamesPath + games[gameID] + "_lvl" + level +".txt";
                System.out.println("Client connected with game " + gameID + " level " + level);
                msg.destroy();

                ArcadeMachine.runOneGame(gameString, levelString, visuals, zmqController, recordActionsFile, seed.nextInt(), 0);
            }
        }

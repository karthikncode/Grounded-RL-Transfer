...
function aml:createNetwork(args)
   ...
   for i=1,self.n_games do
      local game_id = self.game_names[i] + 5000
      local netname = args.expertnet_prefix .. 'gvgai_' .. game_id .. '.t7'
      collectgarbage()
      print('Creating Expert Network ' .. i .. ' from ' .. netname .. ' starting from directory' .. paths.cwd())
      local msg, exp = pcall(torch.load, netname)
      if not msg then
         error('Error loading expert network')
      end
      self.expertnet[i] = exp.model
      self.text[i] = exp.text
		-- self.expertnet[i]:cuda()
		self.expertnet[i]:forward({torch.zeros(1, self.objects_per_cell, unpack(self.input_dims)), text_tensor:float()})
      ...
      print('Outsize: ', l.output:size(2), self.n_actions)
...
function aml:calcTargets()
   ...
   for i=1,self.n_games do
      local idx = {{(i-1)*self.minibatch_size+1, i*self.minibatch_size},{}}
		local destate = self.full_s[idx]
      -- print('Debug: ')
      -- print('Debug: The state has size ', destate:size())
      -- print('Debug: The state has type ', torch.type(destate))
      -- print('Debug: The state has first row ', destate[1])
      local expertout = self.expertnet[i]:forward({self.full_s[idx]:float(), text_tensor:float()})[1]:float()
   ...
   local outputs = self.network:forward(self.full_s:float())
...
function aml:forwardNetwork(gameidx, state)
	...
	return self.expertnet[gameidx]:forward(state)[1]:float():squeeze()
end
...
function aml:compute_validation_statistics()
   ...
   local outputs = self.network:forward(self.valid_s:float())
